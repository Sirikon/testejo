...
function Testejo(_element_, _tpath_){
	...
	this.draw = function(){
		...
		if(this._uploadShown){
			this._element.find("*[role='back']").removeClass('hidden');
		}else{
			this._element.find("*[role='back']").addClass('hidden');
		}
		if(this._uploadShown){
			this._element.find("*[role='add']").addClass('hidden');
		}
...
$(document).ready(function(){
	$(document).on('click', "*[tj-file] input[type='checkbox']", function(e){
	...
	$(document).on('click', "*[tj-file]", function(e){
		if(e.target.nodeName != 'INPUT'){
			window.open($(e.target).attr('tj-thumbnail'), '_blank');
		}
	});
	...
	$(document).on('click', "*[role='back']", function(){
		var t = Tj.getTestejo($(this)).get(0).Testejo;
		if(t._loading){ return; }
		t.hideUpload();
	});
	$(document).on('submit', "div.tj-testejo form", function(e){
		e.preventDefault();
		var t = Tj.getTestejo($(this)).get(0).Testejo;
		if(t._loading){ return; }
		...
		if ( input.files && input.files[0] ) {
	        ...
	        FR.onload = function(e) {
	        	if(e.loaded > (2 * 1024 * 1024)){
	        		setTimeout(function(){
	        			alert("El tamaño máximo de las imágenes es de 2MB");
	        		}, 500);
	        	}else{
	        		t.loadingOn();
	        		t.new(e.target.result, function(newElement){
	        			t._content.append(t._render('file',newElement));
	        			t.hideUpload();
	        			t.loadingOff();
	        		});
	        	}
